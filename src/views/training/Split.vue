<template>
  <v-sheet class="d-flex flex-column" rounded="lg">
    <v-sheet
      class="d-flex align-center justify-space-between ma-3"
      color="transparent"
    >
      <v-card
        elevation="0"
        :title="
          'Woche vom Montag ' + weekdays[0].date.toLocaleDateString('de-DE')
        "
      ></v-card>
      <v-sheet class="d-flex">
        <v-btn
          ...
          class="mr-3"
        ></v-btn>
        <v-btn icon="mdi-calendar-arrow-right" @click="nextWeek"></v-btn>
      </v-sheet>
    </v-sheet>
    <SplitDay
